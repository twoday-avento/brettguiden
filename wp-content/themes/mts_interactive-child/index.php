        ...
        while ($my_query->have_posts()) : $my_query->the_post();
            ...
            <?php if ($i == 1) { ?>
                <?php if (has_post_thumbnail()) {
                    $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'featured-post-1');
                } ?>
				<article<?php echo $featured_class; ?> style="background-image: url('<?php echo $featured_image[0]; ?>');" itemscope itemtype="http://schema.org/BlogPosting">
					...
					<a href="<?php echo esc_url(get_category_link(4)); ?>" title="<?php the_title(); ?>">
						<header>
							<div class="post-info">
								<span class="thecategory"><?php $category = get_the_category();
            ...
            <?php if ($i == 2) { ?>
                <?php if (has_post_thumbnail()) {
                    $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'featured-post-2');
                } ?>
				<article<?php echo $featured_class; ?> style="background-image: url('<?php echo $featured_image[0]; ?>');" itemscope itemtype="http://schema.org/BlogPosting">
					<meta itemprop="image" content="<?php echo esc_attr($featured_image[0]); ?>">
					<a href="<?php echo esc_url(get_category_link(4)); ?>" title="<?php the_title(); ?>">
            ...
            <?php if ($i == 3 || $i == 4) { ?>
                <?php if (has_post_thumbnail()) {
                    $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'featured-post-3');
                } ?>
				<article<?php echo $featured_class; ?> style="background-image: url('<?php echo $featured_image[0]; ?>');" itemscope itemtype="http://schema.org/BlogPosting">
					<meta itemprop="image" content="<?php echo esc_attr($featured_image[0]); ?>">
					<a href="<?php echo esc_url(get_category_link(4)); ?>" title="<?php the_title(); ?>">
            ...
            if ($cat_query->have_posts()) : while ($cat_query->have_posts()) : $cat_query->the_post(); ?>
				<article class="latestPost" itemscope itemtype="http://schema.org/BlogPosting">
					<a href="<?php echo esc_url(get_the_permalink()); ?>" title="<?php echo esc_attr(get_the_title()); ?>" rel="nofollow" class="post-image post-image-left">
                        ...
                        the_post_thumbnail('featured-cat', array('title' => '', 'itemprop' => 'image'));
                        echo '</div>'; ?>
                        <?php if (function_exists('wp_review_show_total')) wp_review_show_total(true, 'latestPost-review-wrapper'); ?>
						<header>
							<div class="thecategory"><?php $category = get_the_category();
...
<?php } ?>
	...
	<div class="sidebar-with-content">
        <?php get_template_part('parts/block', 'homepage_games') ?>
                        ...
                        foreach ($mts_options['mts_featured_categories'] as $section) {
                                    ...
                                    <?php if (have_posts()) : while (have_posts()) : the_post();
                                        ...
                                        if ($i == 1) { ?>
											<article class="latestPost excerpt grid-<?php echo $i; ?>" itemscope itemtype="http://schema.org/BlogPosting">
												<a href="<?php echo esc_url(get_the_permalink()); ?>" title="<?php echo esc_attr(get_the_title()); ?>" rel="nofollow" class="post-image post-image-left">
                                                    <?php echo '<div class="featured-thumbnail">';
                                                    the_post_thumbnail('featuredfull', array('title' => '', 'itemprop' => 'image'));
                                                    echo '</div>'; ?>
                                                    <?php if (function_exists('wp_review_show_total')) wp_review_show_total(true, 'latestPost-review-wrapper'); ?>
												</a>
												<header>
                                        ...
                                        <?php if ($i >= 2) { ?>
											<article class="latestPost excerpt grid-2" itemscope itemtype="http://schema.org/BlogPosting">
												<a href="<?php echo esc_url(get_the_permalink()); ?>" title="<?php echo esc_attr(get_the_title()); ?>" rel="nofollow" class="post-image">
                                                    <?php echo '<div class="featured-thumbnail">';
                                                    the_post_thumbnail('featuredmedium', array('title' => '', 'itemprop' => 'image'));
                                                    echo '</div>'; ?>
                                                    <?php if (function_exists('wp_review_show_total')) wp_review_show_total(true, 'latestPost-review-wrapper'); ?>
												</a>
												<header>
                            ...
                            <?php } else { ?>
								<h3 class="featured-category-title">
									<a href="<?php echo esc_url(get_category_link($category_id)); ?>" title="<?php echo esc_attr(get_cat_name($category_id)); ?>"><?php echo esc_html(get_cat_name($category_id)); ?></a>
								</h3>
                                <?php
                                $j = 0;
