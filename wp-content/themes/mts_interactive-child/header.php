...
<head itemscope itemtype="http://schema.org/WebSite">
    ...
    <?php wp_head(); ?>
    <link rel='stylesheet' id='main_style-css' href='<?php echo get_stylesheet_directory_uri(); ?>/css/main.css' type='text/css' media='all'/>
		...
		<header id="site-header" role="banner" itemscope itemtype="http://schema.org/WPHeader">
							...
							<?php if( is_front_page() || is_home() || is_404() ) { ?>
								<h1 id="logo" class="image-logo" itemprop="headline">
									<a href="<?php echo esc_url( home_url() ); ?>">
										<img src="<?php echo esc_url( home_url() ); ?>/wp-content/uploads/2016/11/logo-1.png" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"<?php if (!empty($mts_options['mts_logo2x'])) { echo ' data-at2x="'.esc_attr( $mts_options['mts_logo2x'] ).'"'; } ?>>
									</a>
								</h1><!-- END #logo -->
							<?php } else { ?>
							  	<h1 id="logo" class="image-logo" itemprop="headline">
            ...
            <?php if ( !empty($mts_options['mts_latest_categories']) && $mts_options['mts_latest_categories_show_hide'] == 1 ) { ?>
                <div class="latestPost-categories">
                    <?php displayLatestCategories($mts_options) ?>
                </div>
            <?php } ?>
    </header>
